  ...
  config = function()
    vim.api.nvim_create_autocmd('LspAttach', {
      ...
      callback = function(event)
        ...
        -- Wrapper for telescope LSP functions with position encoding
        local function tel(fn)
          return function()
            local enc = (client and client.offset_encoding) or 'utf-16'
            require('telescope.builtin')[fn] { position_encoding = enc }
          end
        end

        -- Navigation (use vim.lsp.buf.definition directly - telescope ignores position_encoding)
    ...
    -- Server configurations (keys: cmd, filetypes, capabilities, settings)
    local servers = {
      clangd = {},
      pyright = {},
      html = {},
